...
def start():
    global P_max,vel_bola,vel_barra
    system('cls')
    try:
        P_max = int(input("Digite a pontuação máxima: "))
        vel_bola = int(input('Digite a velocidade da bola: '))
        vel_barra = int(input("Digite a velocidade da barra: "))
    except:
        P_max = 5
        vel_bola = 15
        vel_barra = 10
...
yb = int(height/2)
b_radius = 10
velx_bola = random.randint(int(3*vel_bola/7),int(5*vel_bola/7))
vely_bola = math.sqrt(vel_bola*vel_bola+velx_bola*velx_bola)
B0 = 0
